...
export default function FinancePage() {
    ...
    const changeTab=(value:FinanceTab)=>{const next=new URLSearchParams(params);next.set('view',value);setParams(next);setStatus('');setCurrency('');setSearch('');};
    const openById=(id:string)=>{const invoice=invoices.find((item)=>item.id===id);if(invoice)setSelected(invoice);};
    ...
    return <div className="space-y-5"><PageHeader title="ROTERO Finance 360" subtitle="Operación → AR/AP → pagos → saldos → rentabilidad"/>
        ...
        {!tenantId?<div className="rounded-2xl border bg-white p-12 text-center text-sm text-slate-400">Selecciona una organización activa.</div>:loading&&!overview?<div className="flex justify-center rounded-2xl border bg-white p-20"><Loader2 className="animate-spin text-primary"/></div>:<>
            ...
            {tab==='ar'&&<ReceivablesWorkspace invoices={filtered} search={search} status={status} currency={currency} onSearch={setSearch} onStatus={setStatus} onCurrency={setCurrency} onOpen={setSelected} onPay={setPaying} onCreate={()=>setCreating('ar')}/>}
            {tab==='ap'&&<PayablesWorkspace invoices={filtered} search={search} status={status} currency={currency} onSearch={setSearch} onStatus={setStatus} onCurrency={setCurrency} onOpen={setSelected} onPay={setPaying} onCreate={()=>setCreating('ap')}/>}
