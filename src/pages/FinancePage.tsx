...
export default function FinancePage() {
    ...
    ]);setOverview(nextOverview);setInvoices(nextInvoices);setPayments(nextPayments);setAlerts(nextAlerts);setProfitability(nextProfitability);setOperations(nextOperations);}catch(cause){setError(cause instanceof Error?cause.message:'No fue posible cargar Finanzas.');}finally{setLoading(false);}},[tenantId,canView,tab,search,status,currency,daysAhead,customerId,providerId]);
    ...
    if(!canView)return <div className="space-y-6"><PageHeader title="Finanzas" subtitle="Control financiero operativo"/><SemanticPanel tone="warning" className="rounded-2xl p-10 text-center"><ShieldAlert className={`mx-auto ${SEMANTIC_TONE_STYLES.warning.accent}`}/><h2 className="mt-3 font-black text-slate-800">Acceso financiero restringido</h2><p className="mt-1 text-sm text-slate-500">Sólo Administración y Finanzas pueden consultar o modificar este módulo.</p></SemanticPanel></div>;

    return <div className="min-w-0 max-w-full space-y-4 sm:space-y-5"><PageHeader title="Finanzas" subtitle="Operaciones, cuentas, pagos, saldos y rentabilidad" actions={<SavedViewsMenu tenantId={tenantId} module="finance" filters={{view:tab,status,currency,search}} onApply={(filters)=>{if(typeof filters.view==='string'&&isTab(filters.view)){const next=new URLSearchParams(params);next.set('view',filters.view);next.delete('invoiceId');setParams(next,{replace:true});}setStatus(typeof filters.status==='string'?filters.status as InvoiceStatus:'');setCurrency(typeof filters.currency==='string'?filters.currency as FinanceCurrency:'');setSearch(typeof filters.search==='string'?filters.search:'');}}/>}/>
        <div className="min-w-0 rounded-2xl border bg-surface-card p-2 sm:p-4"><div className="flex min-w-0 flex-col gap-3 xl:flex-row xl:items-center xl:justify-between"><nav aria-label="Secciones de Finanzas" className="flex max-w-full gap-1 overflow-x-auto overscroll-x-contain pb-1 sm:flex-wrap sm:gap-2">{TABS.map((item)=><button key={item.value} onClick={()=>changeTab(item.value)} aria-pressed={tab===item.value} className={`min-h-11 shrink-0 rounded-xl px-3 text-xs font-bold sm:px-3.5 ${tab===item.value?'bg-primary text-white':'bg-surface text-slate-500 hover:text-primary'}`}>{item.label}</button>)}</nav><div className="flex gap-2 px-1 pb-1 sm:px-0 sm:pb-0"><button onClick={()=>void exportCsv()} className="inline-flex min-h-11 items-center gap-2 rounded-xl border px-3 text-xs font-bold text-slate-600"><Download size={14}/>CSV</button><button onClick={()=>void load()} aria-label="Actualizar Finanzas" className="inline-flex min-h-11 items-center gap-2 rounded-xl border px-3 text-xs font-bold text-slate-600"><RefreshCw size={14}/><span className="hidden sm:inline">Actualizar</span></button></div></div></div>
        <SemanticPanel tone="info" className="flex min-w-0 items-start gap-2 px-4 py-3 text-xs"><ShieldAlert size={16} className={`mt-0.5 shrink-0 ${SEMANTIC_TONE_STYLES.info.accent}`}/><p className="min-w-0 break-words leading-relaxed text-slate-600"><strong className={SEMANTIC_TONE_STYLES.info.accent}>Frontera fiscal:</strong> Finanzas registra cuentas, saldos y la preparación del complemento. El timbrado, la cancelación fiscal y la ejecución ante SAT continúan en Facturación.</p></SemanticPanel>
