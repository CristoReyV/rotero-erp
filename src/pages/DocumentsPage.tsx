...
import type { DocumentEntityType, DocumentFile, DocumentFileFilters, DocumentFileKind, DocumentFileStatus, DocumentSourceOption, DocumentTemplateType, GeneratedDocument } from '@/types/documents';
...
export default function DocumentsPage() {
    const tenantId = useAuthStore((state) => state.activeTenant); const role = useAuthStore((state) => state.getRole()); const [params,setParams]=useSearchParams(); const requested=params.get('view'); const view:View=VIEWS.some(item=>item.id===requested)?requested as View:'all';
    ...
    return <div className="space-y-5"><PageHeader title="Documents 360" subtitle="Archivos reales, expediente operativo y documentos generados" actions={<SavedViewsMenu tenantId={tenantId} module="documents" filters={{view,search,kind,status:statusFilter,dateFrom,dateTo}} onApply={(filters)=>{if(typeof filters.view==='string'&&VIEWS.some(item=>item.id===filters.view))setView(filters.view as View);setSearch(typeof filters.search==='string'?filters.search:'');setKind(typeof filters.kind==='string'?filters.kind as DocumentFileKind:'');setStatusFilter(typeof filters.status==='string'?filters.status as DocumentFileStatus:'');setDateFrom(typeof filters.dateFrom==='string'?filters.dateFrom:'');setDateTo(typeof filters.dateTo==='string'?filters.dateTo:'');}}/>} /><nav className="flex gap-1 overflow-x-auto rounded-2xl border bg-white p-1.5">{VIEWS.map((item) => <button key={item.id} onClick={() => setView(item.id)} className={`whitespace-nowrap rounded-xl px-4 py-2 text-xs font-bold ${view === item.id ? 'bg-primary text-white' : 'text-slate-500'}`}>{item.label}</button>)}</nav>
