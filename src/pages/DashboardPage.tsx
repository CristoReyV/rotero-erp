...
import { ArrowRight, BriefcaseBusiness, CalendarDays, FileCheck2, Loader2, MessageSquareWarning, RefreshCw, ShieldCheck, Wallet } from 'lucide-react';
...
import { getClaimsDashboard, listClaimsAttentionItems } from '@/services/claims.service';
...
import type { ClaimsDashboard } from '@/types/claims';
...
export default function DashboardPage(){
    ...
    const [claims,setClaims]=useState<ClaimsDashboard|null>(null);
    const load=useCallback(async()=>{if(!tenantId)return;setLoading(true);setError(null);try{const range=resolveRange(preset,from,to);const[dashboard,complianceSummary,complianceAttention,claimsSummary,claimsAttention]=await Promise.all([getExecutiveDashboard(tenantId,range.start,range.end),role==='admin'?getComplianceDashboard(tenantId):Promise.resolve(null),role==='admin'?listComplianceAttentionItems(tenantId):Promise.resolve([]),role==='admin'?getClaimsDashboard(tenantId,range.start,range.end):Promise.resolve(null),role==='admin'?listClaimsAttentionItems(tenantId):Promise.resolve([])]);setData({...dashboard,attention:[...claimsAttention,...complianceAttention,...dashboard.attention]});setCompliance(complianceSummary);setClaims(claimsSummary);}catch(cause){setError(cause instanceof Error?cause.message:'No fue posible cargar el panel ejecutivo.');}finally{setLoading(false);}},[tenantId,preset,from,to,role]);
    ...
    return <div className="min-w-0 max-w-full space-y-5"><header className="flex min-w-0 flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div className="min-w-0"><p className="text-xs font-black uppercase tracking-[.22em] text-primary">Centro de control ROTERO</p><h1 className="mt-1 text-2xl font-black text-slate-900">Panel ejecutivo</h1><p className="mt-1 text-sm text-slate-500">Estado real y acciones prioritarias para {role==='finance'?'Finanzas':'Administración'}.</p></div><div className="flex min-w-0 flex-wrap items-center gap-2"><div className="flex min-w-0 flex-wrap gap-1 rounded-xl border bg-surface-card p-1">{presets.map(item=><button key={item.value} onClick={()=>setPreset(item.value)} className={`rounded-lg px-3 py-2 text-[10px] font-black uppercase ${preset===item.value?'bg-primary text-white':'text-slate-500 hover:bg-slate-50'}`}>{item.label}</button>)}</div><button onClick={()=>void load()} className="rounded-xl border bg-surface-card p-2.5 text-slate-500" title="Actualizar"><RefreshCw size={16} className={loading?'animate-spin':''}/></button></div></header>
        ...
        {loading&&!data?<div className="flex h-64 items-center justify-center rounded-2xl border bg-white"><Loader2 className="animate-spin text-primary"/></div>:data&&<>
            ...
            <div className="grid min-w-0 max-w-full gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(360px,.85fr)]"><AttentionCenter items={data.attention}/><RecentActivity items={data.recent_activity}/></div>
            <div className={`grid gap-4 ${data.commercial?'xl:grid-cols-5':'xl:grid-cols-3'}`}>
                <Summary icon={BriefcaseBusiness} title="Operaciones" route="/operations" onOpen={navigate} rows={[["En tránsito",data.operations.in_transit],["Entregadas",data.operations.delivered],["Bloqueos",data.operations.dispatch_blockers]]}/>
                {data.commercial&&<Summary icon={ArrowRight} title="Comercial" route="/commercial?view=quotes" onOpen={navigate} rows={[["En revisión",data.commercial.in_review],["Por convertir",data.commercial.pending_conversion],["Conversión real",`${data.commercial.conversion_rate}%`]]}/>}
                <Summary icon={Wallet} title="Finanzas" route="/finance" onOpen={navigate} rows={[["Por cobrar vencido",money(data.finance.ar_overdue)],["Por pagar vencido",money(data.finance.ap_overdue)],["Vence pronto",data.finance.due_soon]]}/>
                <Summary icon={FileCheck2} title="Documentos" route="/documents?view=operations" onOpen={navigate} rows={[["Requeridos faltantes",data.documents.required_missing],["POD pendientes",data.documents.pod_pending],["Facturación bloqueada",data.operations.billing_blocked]]}/>
                {role==='admin'&&compliance&&<Summary icon={ShieldCheck} title="Cumplimiento" route="/commercial?view=compliance" onOpen={navigate} rows={[["Proveedores bloqueados",compliance.blocked_providers],["Documentos 30d",compliance.documents_expiring_30d],["Contratos 30d",compliance.contracts_expiring_30d]]}/>}
                {role==='admin'&&claims&&<Summary icon={MessageSquareWarning} title="Reclamaciones" route="/claims?view=open" onOpen={navigate} rows={[["Abiertas",claims.open_count],["Críticas",claims.critical_open],...[...claims.exposure_by_currency].map(item=>[`Exposición ${item.currency}`,`${item.currency} ${Number(item.amount).toLocaleString()}`] as [string,string])]}/>}
            </div>
        </>}
    </div>;
}
