...
export default function DashboardPage(){
    ...
    const load=useCallback(async()=>{if(!tenantId)return;setLoading(true);setError(null);try{const range=resolveRange(preset,from,to);const[dashboard,complianceSummary,complianceAttention,claimsSummary,claimsAttention]=await Promise.all([getExecutiveDashboard(tenantId,range.start,range.end),role==='admin'?getComplianceDashboard(tenantId):Promise.resolve(null),role==='admin'?listComplianceAttentionItems(tenantId):Promise.resolve([]),role==='admin'?getClaimsDashboard(tenantId,range.start,range.end):Promise.resolve(null),role==='admin'?listClaimsAttentionItems(tenantId):Promise.resolve([])]);setData({...dashboard,attention:[...claimsAttention,...complianceAttention,...dashboard.attention]});setCompliance(complianceSummary);setClaims(claimsSummary);}catch(cause){setError(cause instanceof Error?cause.message:'No fue posible cargar Executive Dashboard.');}finally{setLoading(false);}},[tenantId,preset,from,to,role]);
    ...
    return <div className="space-y-5"><header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div><p className="text-xs font-black uppercase tracking-[.22em] text-primary">ROTERO Command Center</p><h1 className="mt-1 text-2xl font-black text-slate-900">Executive Dashboard 2.0</h1><p className="mt-1 text-sm text-slate-500">Estado real y acciones prioritarias para {role==='finance'?'Finanzas':'Administración'}.</p></div><div className="flex flex-wrap items-center gap-2"><div className="flex flex-wrap gap-1 rounded-xl border bg-white p-1">{presets.map(item=><button key={item.value} onClick={()=>setPreset(item.value)} className={`rounded-lg px-3 py-2 text-[10px] font-black uppercase ${preset===item.value?'bg-primary text-white':'text-slate-500 hover:bg-slate-50'}`}>{item.label}</button>)}</div><button onClick={()=>void load()} className="rounded-xl border bg-white p-2.5 text-slate-500" title="Actualizar"><RefreshCw size={16} className={loading?'animate-spin':''}/></button></div></header>
        ...
        {loading&&!data?<div className="flex h-64 items-center justify-center rounded-2xl border bg-white"><Loader2 className="animate-spin text-primary"/></div>:data&&<>
            ...
            <div className={`grid gap-4 ${data.commercial?'xl:grid-cols-5':'xl:grid-cols-3'}`}>
                ...
                {role==='admin'&&claims&&<Summary icon={MessageSquareWarning} title="Claims" route="/claims?view=open" onOpen={navigate} rows={[["Abiertas",claims.open_count],["Críticas",claims.critical_open],...[...claims.exposure_by_currency].map(item=>[`Exposición ${item.currency}`,`${item.currency} ${Number(item.amount).toLocaleString()}`] as [string,string])]}/>}
            </div>
        </>}
    </div>;
}
