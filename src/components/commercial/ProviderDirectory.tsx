...
export function ProviderDirectory({ tenantId, requestedProviderId, createRequested = false, onProviderChange, onCreateHandled }: { tenantId: string; requestedProviderId?: string | null; createRequested?: boolean; onProviderChange?: (providerId: string | null) => void; onCreateHandled?: () => void }) {
    ...
    useEffect(() => {
        if (!requestedProviderId || selected?.id === requestedProviderId) return;
        const match = items.find((item) => item.id === requestedProviderId);
        if (match) setSelected(match);
    }, [items, requestedProviderId, selected?.id]);
    ...
    return (
        <div className="space-y-4">
            <div className="flex min-w-0 flex-col gap-3 rounded-2xl border bg-surface-card p-3 sm:p-4 lg:flex-row lg:items-center">
                <form onSubmit={(event) => { event.preventDefault(); void load(); }} className="flex min-w-0 flex-1 gap-2"><div className="relative min-w-0 flex-1"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar proveedor, RFC o contacto" className="min-h-11 w-full rounded-xl border bg-surface py-2.5 pl-9 pr-3 text-sm" /></div><button className="min-h-11 shrink-0 rounded-xl border px-3 text-xs font-bold text-slate-600 sm:px-4">Buscar</button></form>
                <div className="flex min-w-0 flex-wrap gap-2"><select value={activeFilter} onChange={(event) => setActiveFilter(event.target.value as typeof activeFilter)} className="min-h-11 min-w-0 flex-1 rounded-xl border bg-surface-card px-3 text-xs font-semibold"><option value="all">Todos</option><option value="active">Activos</option><option value="inactive">Inactivos</option></select><button onClick={() => openForm()} className="flex min-h-11 shrink-0 items-center gap-2 rounded-xl bg-primary px-4 text-xs font-bold text-white"><Plus size={15} /> Nuevo proveedor</button></div>
            </div>
            ...
            {selected && <><Partner360Panel tenantId={tenantId} entityType="provider" entityId={selected.id}/><EntityDocumentsPanel tenantId={tenantId} sourceModule="commercial" entityType="provider" entityId={selected.id} title="Documentos del proveedor" fileKind="provider_upload" /></>}
            <div className="grid gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(320px,.75fr)]">
                <section className="overflow-hidden rounded-2xl border bg-surface-card">{loading ? <div className="flex h-48 items-center justify-center"><Loader2 className="animate-spin text-primary" /></div> : items.length === 0 ? <div className="flex h-48 flex-col items-center justify-center gap-2 text-slate-400"><Inbox /><p className="text-sm">No hay proveedores con estos filtros.</p></div> : <div className="divide-y">{items.map((provider) => <button key={provider.id} onClick={() => { setSelected(provider); onProviderChange?.(provider.id); }} className="grid min-h-[6rem] w-full min-w-0 gap-2 p-3 text-left hover:bg-slate-50 sm:p-4 md:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)_auto] md:items-center md:gap-3"><div className="min-w-0"><div className="flex items-center gap-2"><Building size={16} className="text-primary" /><p className="truncate font-bold text-slate-800">{provider.display_name}</p></div><p className="mt-1 truncate text-xs text-slate-400">{provider.legal_name || provider.tax_id || 'Proveedor externo'}</p></div><div className="min-w-0 text-xs text-slate-500"><p className="truncate">{provider.contact_name || 'Sin contacto'}</p><p className="truncate">{provider.contact_email || provider.contact_phone || 'Sin datos de contacto'}</p></div><div className="text-right"><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${provider.is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>{provider.is_active ? 'Activo' : 'Inactivo'}</span><p className="mt-1 text-[11px] text-slate-400">{provider.quote_count} cot. · {provider.operation_count} op.</p></div></button>)}</div>}</section>
