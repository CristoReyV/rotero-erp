...
import { getOperationStatus } from '@/components/operations/operationsControl';
            ...
            <div className="grid gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(320px,.75fr)]">
                ...
                <aside className="rounded-2xl border bg-white p-5">
                    {detailLoading ? <div className="flex h-40 items-center justify-center"><Loader2 className="animate-spin text-primary" /></div> : !selected ? (
                        <div className="flex h-40 flex-col items-center justify-center gap-2 text-center text-slate-400"><Building2 /><p className="text-sm">Selecciona un cliente para ver su historial.</p></div>
                    ) : (
                        <div className="space-y-5">
                            <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wider text-primary">Cliente</p><h2 className="text-xl font-bold text-slate-800">{selected.customer.display_name}</h2><p className="text-xs text-slate-400">{selected.customer.tax_id || 'RFC no capturado'}</p></div><button onClick={() => openForm(selected.customer)} className="rounded-lg border p-2 text-slate-500"><Edit3 size={15} /></button></div>
                            <div className="grid grid-cols-3 gap-2">{[['Oportunidades', selected.summary.deal_count], ['Cotizaciones', selected.summary.quote_count], ['Operaciones', selected.summary.operation_count]].map(([label, value]) => <div key={label} className="min-w-0 rounded-xl bg-slate-50 p-3 text-center"><p className="text-lg font-bold text-slate-800">{value}</p><p className="break-words text-[10px] text-slate-400">{label}</p></div>)}</div>
                            ...
                            <div><p className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-400">Actividad reciente</p>{selected.operations.length === 0 && selected.quotes.length === 0 ? <p className="text-xs text-slate-400">Aún no hay cotizaciones u operaciones relacionadas.</p> : <div className="space-y-2">{selected.operations.slice(0, 3).map((operation) => <div key={operation.id} className="flex min-w-0 justify-between gap-2 rounded-lg border p-2 text-xs"><span className="min-w-0 truncate font-bold text-slate-700">{operation.reference_code}</span><span className="shrink-0 text-slate-400">{getOperationStatus(operation.status).label}</span></div>)}</div>}</div>
                        </div>
                    )}
                </aside>
            </div>
