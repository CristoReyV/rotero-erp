...
const BASE_TABS: { value: Tab; label: string }[] = [
    ...
    { value: 'timeline', label: 'Timeline' }, { value: 'incidents', label: 'Incidencias' },
    { value: 'documents', label: 'Documentos' }, { value: 'evidence', label: 'Evidencias' },
    { value: 'economics', label: 'Economía' },
];

export function Operation360Panel({ operation, canManage, canManageTracking, isAdmin, refreshKey, onClose, onAssign, onGenerateTokens, onTransition, onOverrideCancel, onOperationsRefresh }: {
    ...
    const load = useCallback(async () => { if (!operation.db_id) return; setLoading(true); setError(null); try { setData(await getOperation360Data(operation.db_id)); } catch (cause) { setError(cause instanceof Error ? cause.message : 'No fue posible cargar Operation 360.'); } finally { setLoading(false); } }, [operation.db_id]);
    ...
    return <div className="fixed inset-0 z-40 bg-slate-950/45 p-2 backdrop-blur-sm sm:p-4" role="dialog" aria-modal="true" aria-label={`Operation 360 ${operation.id}`}>
        <div className="ml-auto flex h-full w-full max-w-7xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
            <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-5 py-4"><div><div className="flex items-center gap-2"><h2 className="text-xl font-black text-slate-900">Operation 360 · {operation.id}</h2><Badge variant={status.variant}>{status.label}</Badge></div><p className="mt-1 text-xs text-slate-400">Ejecución contratada y expediente operativo centralizado</p></div><div className="flex items-center gap-2"><button onClick={() => void refresh()} className="rounded-lg border border-slate-200 p-2 text-slate-500" title="Actualizar"><RefreshCw size={16} /></button><button onClick={onClose} className="rounded-lg bg-slate-100 p-2 text-slate-600" aria-label="Cerrar Operation 360"><X size={18} /></button></div></header>
            <nav className="flex gap-1 overflow-x-auto border-b border-slate-200 px-4 py-2">{tabs.map((item) => <button key={item.value} onClick={() => setTab(item.value)} className={`whitespace-nowrap rounded-lg px-3 py-2 text-xs font-bold ${tab === item.value ? 'bg-primary text-white' : 'text-slate-500 hover:bg-slate-100'}`}>{item.label}</button>)}</nav>
            <main className="flex-1 overflow-y-auto p-4 sm:p-6">
                {loading ? <div className="flex h-full items-center justify-center gap-2 text-sm text-slate-400"><Loader2 className="animate-spin" />Cargando expediente operativo…</div> : error && !data ? <div className="mx-auto max-w-xl rounded-2xl border border-red-200 bg-red-50 p-8 text-center"><AlertTriangle className="mx-auto text-red-500" /><p className="mt-3 text-sm font-semibold text-red-700">{error}</p><button onClick={() => void load()} className="mt-4 rounded-lg bg-red-600 px-4 py-2 text-xs font-bold text-white">Reintentar</button></div> : data && <div className="space-y-5">
                    ...
                    {tab === 'overview' && <><OperationOverview operation={data.operation} /><section className="border-t border-slate-100 pt-5"><h3 className="mb-3 font-bold text-slate-800">Readiness de despacho y cierre</h3><OperationReadiness data={data} canManageTracking={canManageTracking} onGenerateTokens={() => void generate()} /></section>{canManage && <div className="flex flex-wrap gap-2 border-t border-slate-100 pt-5">{nextStatus[data.operation.status] && <button disabled={actionLoading} onClick={() => void transition(nextStatus[data.operation.status]!)} className="rounded-xl bg-primary px-4 py-2.5 text-xs font-bold text-white">Avanzar a {getOperationStatus(nextStatus[data.operation.status]!).label}</button>}{isAdmin && data.operation.status !== 'cancelled' && data.operation.status !== 'closed' && <button onClick={onOverrideCancel} className="inline-flex items-center gap-2 rounded-xl bg-red-50 px-4 py-2.5 text-xs font-bold text-red-700"><Ban size={14} />Cancelación con motivo</button>}</div>}<div className="grid gap-3 border-t border-slate-100 pt-5 md:grid-cols-2"><div className="rounded-xl border border-dashed border-slate-300 p-4"><div className="flex items-center gap-2"><Bell size={16} className="text-slate-500" /><p className="text-xs font-bold text-slate-700">Notificaciones internas</p></div><p className="mt-2 text-xs text-slate-500">La infraestructura actual no expone un fetch operation-scoped reproducible; se conserva el enlace por ruta para F5/F6 sin duplicar tablas.</p></div><div className="rounded-xl border border-dashed border-slate-300 p-4"><div className="flex items-center gap-2"><FileOutput size={16} className="text-slate-500" /><p className="text-xs font-bold text-slate-700">Generación documental</p></div><p className="mt-2 text-xs text-slate-500">Los templates/generated_documents históricos no están canonizados en reset local; el acceso formal continúa en F3.</p></div></div></>}
                    ...
                    {tab === 'incidents' && <OperationIncidents data={data} canManage={canManage} onRefresh={refresh} />}
