...
function DocumentEditor({ tenantId, document, files, canManage, onRefresh }: { key?: string; tenantId: string; document: OperationDocument; files: DocumentFile[]; canManage: boolean; onRefresh: () => Promise<void> }) {
    ...
    const tone=missing?'danger':document.status==='present'?'success':'neutral';
    return <article className={`min-w-0 space-y-3 rounded-2xl border bg-surface-card p-4 ${SEMANTIC_TONE_STYLES[tone].border}`}>
...
export function OperationDocuments({ data, canManage, onRefresh }: { data: Operation360Data; canManage: boolean; onRefresh: () => Promise<void> }) {
    ...
    if (!operationId || !tenantId) return <p className="rounded-xl border border-dashed p-6 text-sm text-slate-400">La operación no tiene un expediente documental disponible.</p>;
    return <div className="space-y-4">
        <div className="flex min-w-0 flex-wrap items-center justify-between gap-3"><SemanticPanel tone={data.documentSummary.pod_present ? 'success' : 'warning'} className="min-w-0 flex-[1_1_16rem] rounded-2xl p-4"><p className="text-sm font-bold text-slate-800">POD: {data.documentSummary.pod_present ? 'Presente' : 'Pendiente'}</p><p className="mt-1 break-words text-xs leading-relaxed text-slate-500">{data.documentSummary.pod_present ? 'La prueba de entrega (POD) está registrada en el expediente privado.' : 'Falta registrar la prueba de entrega (POD) en el expediente privado.'}</p></SemanticPanel>{canManage && <button disabled={generating} onClick={() => void createGenerated()} className="inline-flex min-w-0 items-center gap-2 rounded-xl bg-primary px-4 py-3 text-xs font-bold text-white disabled:opacity-50">{generating ? <Loader2 size={15} className="animate-spin" /> : <FileOutput size={15} />}Generar documento operativo</button>}</div>
