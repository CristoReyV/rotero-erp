...
export function OperationExecution({ data, canManage, onAssign, onRefresh }: { data: Operation360Data; canManage: boolean; onAssign: () => void; onRefresh: () => Promise<void> }) {
    ...
    return <div className="space-y-5">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-bold text-slate-800">Planeación operativa</h3><p className="text-xs text-slate-400">Contrato `rpc_complete_operation_planning_v2`.</p></div>{canManage && <button type="button" onClick={() => setEditing((value) => !value)} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold text-slate-600"><PencilLine size={14} />{editing ? 'Cancelar edición' : 'Editar planeación'}</button>}</div>
