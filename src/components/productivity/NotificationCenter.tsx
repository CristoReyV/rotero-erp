...
import type { InternalNotification } from '@/types/executive';
...
export function NotificationCenter({ tenantId }: { tenantId:string|null }) {
    ...
    const navigate=useNavigate(); const {pathname}=useLocation(); const [open,setOpen]=useState(false); const [items,setItems]=useState<InternalNotification[]>([]); const [unread,setUnread]=useState(0); const [loading,setLoading]=useState(false); const ref=useRef<HTMLDivElement>(null);
    const [filter,setFilter]=useState<NotificationFilter>('all');
    const load=useCallback(async(refresh=true)=>{if(!tenantId)return;setLoading(true);try{if(refresh)await refreshInternalNotifications(tenantId);const feed=await listInternalNotifications(tenantId);setItems(feed.items);setUnread(feed.unread_count);}finally{setLoading(false);}},[tenantId]);
    ...
    const visibleItems=items.filter(item=>filter==='all'
        ...
        ||item.module===filter);
    return <div className="relative" ref={ref}><button onClick={()=>setOpen(v=>!v)} className="relative rounded-xl p-2 text-slate-400 hover:bg-primary-50 hover:text-primary" aria-label="Abrir notificaciones"><Bell size={19}/>{unread>0&&<span className="absolute right-0 top-0 flex min-w-4 items-center justify-center rounded-full bg-red-500 px-1 text-[9px] font-black text-white">{Math.min(unread,99)}</span>}</button>{open&&<div className="absolute right-0 top-full z-50 mt-2 w-[min(94vw,440px)] overflow-hidden rounded-2xl border bg-white shadow-2xl"><header className="flex items-center justify-between border-b px-4 py-3"><div><b className="text-sm text-slate-800">Notificaciones</b><p className="text-[10px] text-slate-400">{unread} sin leer</p></div><div className="flex gap-1"><button onClick={()=>void load(true)} title="Actualizar" className="rounded-lg p-2 text-slate-400 hover:bg-slate-100"><RefreshCw size={14} className={loading?'animate-spin':''}/></button>{unread>0&&<button onClick={()=>void markAll()} title="Marcar todas leídas" className="rounded-lg p-2 text-slate-400 hover:bg-slate-100"><CheckCheck size={15}/></button>}</div></header><div className="flex gap-1 overflow-x-auto border-b px-3 py-2">{filters.map(option=><button key={option.value} onClick={()=>setFilter(option.value)} className={`whitespace-nowrap rounded-lg px-2.5 py-1.5 text-[9px] font-black uppercase ${filter===option.value?'bg-primary text-white':'bg-slate-50 text-slate-400'}`}>{option.label}</button>)}</div><div className="max-h-[440px] divide-y overflow-y-auto">{visibleItems.length===0?<div className="p-10 text-center text-xs text-slate-400">Sin notificaciones relevantes.</div>:visibleItems.map(item=>{const Icon=icons[item.module];return <div key={item.id} className={`group flex items-start gap-3 p-4 ${item.read_at?'bg-white':'bg-blue-50/40'}`}><button onClick={()=>void openItem(item)} className="flex min-w-0 flex-1 items-start gap-3 text-left"><span className="rounded-lg bg-slate-100 p-2 text-slate-600">{item.is_automated?<Bot size={14}/>:<Icon size={14}/>}</span><span className="min-w-0"><span className="block truncate text-xs font-black text-slate-800">{item.title}</span><span className="mt-0.5 block text-[11px] leading-relaxed text-slate-500">{item.body}</span><span className="mt-1 block text-[9px] font-bold uppercase text-slate-300">{item.priority} · {new Date(item.occurred_at).toLocaleString('es-MX')}{item.is_automated?` · automática L${item.escalation_level}`:''}</span></span></button><button onClick={()=>void dismiss(item.id)} title="Descartar hasta un nuevo escalamiento o ciclo" className="p-1 text-slate-300 opacity-0 hover:text-red-500 group-hover:opacity-100"><X size={13}/></button></div>})}</div></div>}</div>;
}
